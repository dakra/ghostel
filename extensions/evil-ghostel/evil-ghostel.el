...
the advice is installed on first enable and removed on the last disable."
  ...
  (if evil-ghostel-mode
      (progn
        ...
        ;; suppress ghostel's keyboard-mark->copy-mode switch: otherwise `v'
        ;; activates the mark, `ghostel--mark-activated' flips the buffer to
        ;; read-only copy mode, and the visual operators hit "Buffer is
        ;; read-only".  Mouse selection still picks its own mode independently.
        (setq-local ghostel--inhibit-mark-activation t)
    ...
    (remove-hook 'evil-emacs-state-entry-hook
                 #'evil-ghostel--insert-state-entry t)
    (kill-local-variable 'ghostel--inhibit-mark-activation)
