...
insert/Emacs states and normal-state prompt movement.  Visual range
markers are restored around the native redraw.
  ...
  (if (and evil-ghostel-mode
      ...
      (let* ((visual-p (eq evil-state 'visual))
        ...
        (let* ((post-cursor-line (evil-ghostel--cursor-buffer-line))
          ...
          (when (or prompt-moved (memq evil-state '(insert emacs)))
...
runs `ghostel--redraw-now' synchronously.
