...
(ert-deftest ghostel-test-prompt-nav-honors-readonly-default-mode ()
  "At `default', prompt navigation follows `ghostel-readonly-default-mode'."
  (let ((buf (generate-new-buffer " *ghostel-test-prompt-nav*")))
    (unwind-protect
        (with-current-buffer buf
          ...
          (let ((ghostel--term 'fake)
                (ghostel--redraw-timer nil)
                (ghostel-prompt-navigation-input-mode 'default))
            (cl-letf (((symbol-function 'ghostel--invalidate) #'ignore)
                      ((symbol-function 'ghostel--anchor-window) #'ignore)
                      ((symbol-function 'ghostel-force-redraw) #'ignore)
...
(ert-deftest ghostel-test-prompt-nav-explicit-mode-overrides-default ()
  "An explicit `ghostel-prompt-navigation-input-mode' wins over the default."
  (let ((buf (generate-new-buffer " *ghostel-test-prompt-nav-explicit*")))
    (unwind-protect
        (with-current-buffer buf
          (ghostel-mode)
          (let ((ghostel--term 'fake)
            ...
            (cl-letf (((symbol-function 'ghostel--invalidate) #'ignore)
                      ...
                      ((symbol-function 'ghostel--navigate-next-prompt)
                       (lambda (_n) nil)))
              (let ((ghostel-readonly-default-mode 'copy)
                    (ghostel-prompt-navigation-input-mode 'emacs))
                (ghostel-next-prompt 1)
                (should (eq ghostel--input-mode 'emacs))
                (ghostel-readonly-exit))
              (let ((ghostel-readonly-default-mode 'emacs)
...
(ert-deftest ghostel-test-prompt-nav-preserves-active-readonly-mode ()
  "Prompt navigation in copy or Emacs mode does not switch modes."
  (let ((buf (generate-new-buffer " *ghostel-test-prompt-nav-preserve*")))
    (unwind-protect
        (with-current-buffer buf
          (ghostel-mode)
          (let ((ghostel--term 'fake)
...
(ert-deftest ghostel-test-imenu-goto-honors-prompt-nav-mode ()
  "`ghostel--imenu-goto' enters the configured mode; line mode is preserved."
  (let ((buf (generate-new-buffer " *ghostel-test-imenu-goto*")))
    (unwind-protect
        (with-current-buffer buf
          (ghostel-mode)
          (let ((ghostel--term 'fake)
            ...
            (cl-letf (((symbol-function 'ghostel--invalidate) #'ignore)
                      ...
                      ((symbol-function 'ghostel-force-redraw) #'ignore)
                      ((symbol-function 'ghostel--adjust-size) #'ignore))
              (ghostel--imenu-goto "cmd" (point-min))
              (should (eq ghostel--input-mode 'copy))
              (ghostel-readonly-exit)
