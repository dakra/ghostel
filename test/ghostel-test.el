...
;; -----------------------------------------------------------------------
;; Test: anchored terminal (Shape A prototype)
;; -----------------------------------------------------------------------
    ...
    (unwind-protect
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            ...
            ;; terminal — the Zig constructor takes the start/end
            ;; positions and builds its own markers (end with
            ;; insertion-type t).  If the surrounding text were
            ;; inserted AFTER the terminal was created, the
            ;; insertion-type-t end marker would ride forward
            ;; through "AFTER" and the renderer would clobber it.
            (insert "BEFORE\n")
            (let ((start (point)))
              (insert "\n")
              (let ((end (point)))
...
between the markers must:
- grow past the live grid (live grid + scrollback rows are visible),
- contain rows that have already scrolled off the active area,
- still be flanked by the original BEFORE/AFTER surrounding text.
The renderer's internal markers (stored in the buffer-local hash table
`ghostel--anchored-terminals') must still be markers in the right buffer
with start < end."
    ...
    (unwind-protect
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            (insert "BEFORE-TOP\n")
            (let ((start (point)))
              ;; Use a single newline so the empty region is small —
              ;; this exercises the walk-back math in `redraw' when
              ;; the region is shorter than `scrollbar.len'.
              (insert "\n")
              (let ((end (point)))
                (insert "AFTER-BOTTOM\n")
                ;; 5 rows × 50 KB scrollback budget — easily fits many
                ;; scrolled-off rows but stays bounded.
                (let ((term (ghostel--new-anchored start end 5 20 51200)))
                  ;; Feed 30 numbered lines.  Each `\r\n' advances one
                  ;; grid row; after 30 lines, 25 have scrolled off
                  ;; the 5-row grid.
                  (dotimes (i 30)
                    (ghostel--write-input term (format "line-%02d\r\n" i)))
                  (ghostel--redraw term t)
                  ;; The renderer's own markers are stored in the
                  ;; buffer-local hash table.  Look them up to verify
                  ;; they're still valid markers in this buffer.
                  (let* ((pair (gethash term ghostel--anchored-terminals))
                    ...
                    ;; Renderer markers are still markers, in this
                    ;; buffer, ordered.
                    (should (markerp rs))
                    (should (markerp re))
                    ...
                    ;; Live grid (most recent rows) is present.
                    (should (string-match-p "line-29" mid))
                    (should (string-match-p "line-25" mid))
                    ;; Scrollback materialization: earlier rows that
                    ;; scrolled off the active grid are now in the
                    ;; region too (libghostty kept them in scrollback,
                    ;; and the redraw promoted them into the buffer).
                    (should (string-match-p "line-15" mid))
                    (should (string-match-p "line-10" mid))
                    ;; The region must have grown well past the live
                    ;; grid (5 rows).  Count lines inside the region.
...
The renderer must not grow the region past the active grid: scrolled-off
rows are evicted by libghostty, and the renderer follows by trimming the
buffer.  Verifies the eviction math (`libghostty_rows < rows_in_buffer'
branch in `redraw') works for the anchored case."
        ...
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            ...
            (let ((start (point)))
              ...
              (let ((end (point)))
                (insert "AFTER-BOTTOM\n")
                ;; MAX-SCROLLBACK = 0 disables scrollback entirely.
                (let ((term (ghostel--new-anchored start end 5 20 0)))
                  ;; Feed 30 lines into a 5-row grid.  With no
                  ;; scrollback, 25 lines should be evicted.
                  (dotimes (i 30)
                    (ghostel--write-input term (format "line-%02d\r\n" i)))
                  (ghostel--redraw term t)
                  (let* ((pair (gethash term ghostel--anchored-terminals))
                    ...
                    (should (markerp re))
                    (should (< start-pos end-pos))
                    ;; Live grid present (the last lines before
                    ;; cursor) — only the active 5 rows survive.
                    (should (string-match-p "line-29" mid))
                    ;; Earliest lines were evicted.
                    (should-not (string-match-p "line-00" mid))
                    (should-not (string-match-p "line-10" mid))
                    ;; The region must NOT have grown unboundedly:
                    ;; with no scrollback, the active grid is 5 rows.
                    ;; Allow some slack for the trailing newline.
                    (let ((region-lines (length (split-string mid "\n"))))
                      (should (<= region-lines 7)))))))))
      (kill-buffer buf))))

(ert-deftest ghostel-test-anchored-scrollback-after-text-survives ()
  "AFTER text is pushed down by region growth, never overwritten.
With scrollback enabled, the region grows.  The text after the end
marker must move forward in the buffer but its content must be
        ...
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            ...
            (let ((start (point)))
              (insert "\n")
              (let ((end (point)))
                ;; Distinctive AFTER content to detect any clobber.
                (insert "AFTER-LINE-1\nAFTER-LINE-2\nTAIL\n")
...
The hash table keyed by user-pointer is what makes this work."
    ...
    (unwind-protect
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            ...
            ;;   BOTTOM-OUTER\n
            ;;
            ;; Use integer positions for the boundaries (not markers)
            ;; so the boundary positions aren't perturbed by the
            ;; ensuing `insert' calls.  The Zig constructor builds its
            ;; own internal markers from these integer positions.
            (insert "TOP-OUTER\n")
            (let ((s1 (point)))
              (insert "\n")
              (let ((e1 (point)))
                ...
                (let ((s2 (point)))
                  ...
                  (let ((e2 (point)))
                    (insert "BOTTOM-OUTER\n")
                    ;; Create both terminals.  Each constructor
                    ;; copies START/END into its own marker pair
                    ;; stored on the hash table.
                    (let* ((t1 (ghostel--new-anchored s1 e1 3 20 100))
                           (t2 (ghostel--new-anchored s2 e2 3 20 100))
                           ;; Look up each terminal's renderer markers
                           ;; from the buffer-local hash table.  These
                           ;; are the markers the renderer actually
                           ;; consults via `regionStart' / `regionEnd'.
                      ...
                      ;; Hash table must have exactly two entries.
                      ...
                      (let ((content (buffer-substring-no-properties
                        ...
                        (let ((r1 (buffer-substring-no-properties
                              ...
                              (r2 (buffer-substring-no-properties
                                   (marker-position rs2)
                                   (marker-position re2))))
                          ;; Region 1 still has AAAAA (no clobber).
                          (should (string-match-p "AAAAA" r1))
                          (should-not (string-match-p "BBBBB" r1))
                          ;; Region 2 now has BBBBB.
                          (should (string-match-p "BBBBB" r2))
                          (should-not (string-match-p "AAAAA" r2))))
                      ;; Redraw t1 again — must not be affected by t2.
