...
(ert-deftest ghostel-test-output-drain-is-bounded ()
  "Process output draining stops at `ghostel-output-drain-limit'."
  (with-temp-buffer
    (let ((ghostel--process 'fake-proc)
          (ghostel-output-drain-limit 3)
          (props nil)
          (accept-count 0)
          (accept-args nil))
      (cl-letf (((symbol-function 'process-live-p) (lambda (_) t))
                ((symbol-function 'process-get)
                 (lambda (_proc key) (alist-get key props)))
                ((symbol-function 'process-put)
                 (lambda (_proc key value)
                   (setf (alist-get key props) value)))
                ((symbol-function 'accept-process-output)
                 (lambda (proc seconds &optional millisec just-this-one)
                   (push (list proc seconds millisec just-this-one) accept-args)
                   (cl-incf accept-count)
                   t)))
        (ghostel--drain-process-output 'fake-proc)
        (should (= accept-count 3))
        (should (equal (nreverse accept-args)
                       '((fake-proc 0 nil t)
                         (fake-proc 0 nil t)
                         (fake-proc 0 nil t))))
        (should-not (alist-get 'ghostel--filter-draining props))))))

(ert-deftest ghostel-test-output-drain-skips-reentrant-call ()
  "Process output draining does not start a nested drain loop."
  (with-temp-buffer
    (let ((ghostel--process 'fake-proc)
          (ghostel-output-drain-limit 3)
          (props '((ghostel--filter-draining . t)))
          (accept-count 0))
      (cl-letf (((symbol-function 'process-live-p) (lambda (_) t))
                ((symbol-function 'process-get)
                 (lambda (_proc key) (alist-get key props)))
                ...
                ((symbol-function 'accept-process-output)
                 (lambda (&rest _)
                   ...
                   t)))
        (ghostel--drain-process-output 'fake-proc)
        (should (= accept-count 0))
        (should (alist-get 'ghostel--filter-draining props))))))

(ert-deftest ghostel-test-output-drain-disabled-at-zero ()
  "Process output draining is disabled when the limit is 0."
  (with-temp-buffer
    (let ((ghostel--process 'fake-proc)
          (ghostel-output-drain-limit 0)
          (props nil)
          (accept-count 0))
      (cl-letf (((symbol-function 'process-live-p) (lambda (_) t))
                ((symbol-function 'process-get)
...
(ert-deftest ghostel-test-output-drain-stops-when-process-exits ()
  "Process output draining stops if PROCESS exits during the drain."
  (with-temp-buffer
    (let ((ghostel--process 'fake-proc)
          (ghostel-output-drain-limit 3)
          (props nil)
