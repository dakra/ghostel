...
| =ghostel-output-functions=         | =nil=                                           | Hook run with =(BUFFER)= after rendered terminal output changed the buffer text.  No shell integration needed.  |
...
Outside of a compile buffer, these hooks let you react to shell commands and
terminal output in /any/ ghostel buffer:
...
- =ghostel-output-functions= - called with =(BUFFER)= after rendered terminal
  output changed the buffer text.  Unlike the OSC 133 hooks this needs no
  shell integration, so it also covers REPLs and TUI programs that emit no
  semantic prompt markers.  Calls are coalesced per render batch; snapshot
  =ghostel-cursor-point= before sending input to scan just the output produced
  after it.  Add handlers buffer-locally: a buffer with handlers is rendered
  even while not displayed in any window, so a global handler keeps every
  hidden terminal rendering.  Handlers run from the render path - defer
  anything that sends input back to the terminal, e.g. via =run-at-time=.
