...
=C-x C-q= (the usual read-only toggle) enters your preferred read-only mode -
picked by =ghostel-readonly-default-mode=, =copy= by default - and exits it
again when pressed inside copy or Emacs mode.
...
- =default= (the default) - enter the mode picked by
  =ghostel-readonly-default-mode= (initially =copy=).
...
input mode picked by =ghostel-mark-activation-input-mode= (=default= follows
=ghostel-readonly-default-mode=; =copy=, =emacs=, or =nil= to stay in semi-char).  This hooks mark activation rather than
...
the mode picked by =ghostel-point-leave-input-mode= (=default= follows
=ghostel-readonly-default-mode=; =copy=, =emacs=, or =nil= to disable), so the
position is frozen and the content is
...
that has since moved elsewhere gets a =cd= typed into its shell.  The =cd= is
only typed into an idle shell: not while a command runs (detected through
shell integration) or a full-screen program is open, and not when there is
already input on the prompt line.  Set it to =nil= to leave the directory
alone.
...
credentials.  Terminals whose saved directory no longer exists fail to load,
with an error in =*Messages*=.  Terminals whose shell already exited are not
...
=ghostel-org= adds a =ghostel:= Org link type that names a directory and,
optionally, a buffer:

#+begin_example
[[ghostel:~/src/project][project shell]]
[[ghostel:~/src/project::*ghostel*][my shell, in project]]
#+end_example

Following the first form pops to a live ghostel terminal whose working
directory is that directory, or starts a fresh shell there when none exists.
The second form behaves like a [[#bookmarks][bookmark]]: it switches to the
named buffer, typing a =cd= when it sits elsewhere, and otherwise starts that
buffer in the directory.  =org-store-link= in a ghostel buffer stores the second
form, and =org-insert-link= completes =ghostel:= with a directory prompt.
...
- *Wrapped links* - a URL or path too long for the terminal is split across
  ...
  leaves it intact.
- *thing-at-point and ffap* - =thing-at-point= (=filename=,
  =existing-filename=, =url=) returns the whole target across wrapped rows,
  which =browse-url-at-point=, embark's URL target, and the find-file =M-n=
  default pick up.  =find-file-at-point= is remapped to
  =ghostel-find-file-at-point=, which opens the link at point at its
  recorded line instead of stopping at the row break.
...
- *Virtual placements are stretched to their cell grid.*  A unicode-placeholder
  placement whose =c=/=r= grid has a different aspect ratio than the image is
  scaled to fill the grid, where kitty and Ghostty letterbox it.  =yazi= and
  =timg= size the grid to the image, so the difference stays under one cell.
...
In a terminal Emacs (=emacs -nw=, =emacsclient -t=), =ghostel-tty-forward-notify=
re-emits the notification as OSC 9 / OSC 777 to the outer terminal, so a remote
Emacs over ssh shows the notification on your local desktop.  GUI frames fall
back to =ghostel-default-notify=.  tmux or screen in between drop it; this
handler emits no passthrough wrapper.

#+begin_src emacs-lisp
(setq ghostel-notification-function #'ghostel-tty-forward-notify)
#+end_src
...
| =ghostel-query-before-killing=         | =auto=                                          | Confirm before killing a live buffer / =save-buffers-kill-emacs=: =t=, =nil=, or =auto= (only while a command runs).                                                                                           |
| =ghostel-exit-functions=               | =nil=                                           | Hook run with =(BUFFER EVENT)= when the terminal process exits.                                                                                                                                            |
| =ghostel-command-start-functions=      | =nil=                                           | Hook run with =(BUFFER)= on OSC 133 C (command start).                                                                                                                                                     |
| =ghostel-command-finish-functions=     | =nil=                                           | Hook run with =(BUFFER EXIT-STATUS)= on OSC 133 D (command finish).                                                                                                                                        |
...
| =ghostel-kitty-graphics-mediums=       | all       | Image-loading mediums beyond inline base64, subset of =(file temp-file shared-mem)=; nil accepts inline data only. |
...
| =ghostel-readonly-default-mode=        | =copy=              | Read-only mode entered by =C-x C-q=, hyperlink navigation, and every trigger below left at =default=: =copy= or =emacs=.                                                                                                    |
| =ghostel-mouse-drag-input-mode=        | =default=           | Mode to enter after a click/drag/multi-click selection: =default=, =copy=, =emacs=, or =nil=.                                                                                                                               |
| =ghostel-mark-activation-input-mode=   | =default=           | Mode to enter when a command activates the mark: =default=, =copy=, =emacs=, or =nil=.                                                                                                                                      |
| =ghostel-point-leave-input-mode=       | =default=           | Mode to enter when point leaves the input point (isearch/minibuffer): =default=, =copy=, =emacs=, or =nil=.                                                                                                                 |
| =ghostel-prompt-navigation-input-mode= | =default=           | Mode to enter for prompt navigation (=C-c M-n= / =C-c M-p=, imenu): =default=, =copy=, or =emacs=.                                                                                                                            |
| =ghostel-word-boundary-string=         | (see docstring)   | Characters that terminate words for double-click selection and word motion (mirrors Ghostty's =selection-word-chars=).  Overridden for printable ASCII by =evil-ghostel-word-boundaries= while =evil-ghostel-mode= is on. |
...
:description: Evil, compile, eshell, tmux, comint, consult, and input methods
:end:
...
- Word motions use *Vim-style word boundaries*: =w=, =b=, =e=, =ciw= stop at path
  components (=bar= in =~/src/foo/bar.txt=) instead of treating the whole path
  as one word, matching Vim's default =iskeyword= (=_= stays a word
  constituent).  Configurable via =evil-ghostel-word-boundaries=; set it to
  =nil= to keep ghostel's path-aware boundaries.  Tradeoff: while active,
  every syntax-based word command — double-click, =*=, dabbrev, line-mode
  =M-f=/=M-b= — also sees path components rather than the whole path
  (link clicking, =ghostel-find-file-at-point=, and the whitespace-based
  =W=/=ciW= WORD motions are unaffected), and =ghostel-word-boundary-string=
  only governs non-ASCII characters in these buffers.
...
anything that wants live keystrokes work.  The buffer content stays owned by the
renderer: edits made by Emacs commands are intercepted and forwarded to the
process rather than kept in the buffer.
...
** Tmux integration
:properties:
:custom_id: tmux-integration
:end:
#+cindex: tmux

=ghostel-tmux= connects to an external tmux session via control mode (=tmux -CC=)
and materializes each tmux pane into a real ghostel buffer - full VT engine,
full scrollback, isearch, occur, copy-mode, evil integration, all working on
tmux pane content.  Switching panes is a buffer switch (=tab-line-mode= shows
siblings as tabs).

#+begin_src emacs-lisp
(require 'ghostel-tmux)   ;; autoloaded; require not strictly needed
#+end_src

*** Commands
:properties:
:custom_id: tmux-commands
:end:

| Command                                         | What it does                                                                            |
|-------------------------------------------------+-----------------------------------------------------------------------------------------|
| =M-x ghostel-tmux-new=                            | Create or attach to a session by name (=tmux -CC new-session -As <name>=).               |
| =M-x ghostel-tmux-attach=                         | Pick a running session via =completing-read= and attach.                                 |
| =M-x ghostel-tmux-detach=                         | Detach the current session.  Pane buffers stay as read-only snapshots.                  |
| =M-x ghostel-tmux-capture=                        | One-shot snapshot: dump a single pane's scrollback into a fresh buffer (no live link).  |
| =M-x ghostel-tmux-next-pane= / =ghostel-tmux-prev-pane= | Cycle through sibling panes (also bound to =C-c C-n= / =C-c C-p=).                    |
| =M-x ghostel-tmux-switch-pane=                    | =completing-read= picker over sibling panes (=C-c C-s=).                                  |
| =M-x ghostel-tmux-reset=                          | Escape hatch: reset tmux state when the protocol parser is wedged.                      |

*** Auto-detection: just type =tmux= in a ghostel shell
:properties:
:custom_id: tmux-auto-detection
:end:

The ghostel shell integration wraps =tmux= so attaching subcommands (=tmux new=,
=tmux new -s foo=, =tmux attach=, =tmux a -t foo=, or bare =tmux=) automatically run
with =-CC= (control mode).  The control-mode DCS is then detected (this works in
native-PTY, Emacs-PTY, and TRAMP sessions alike - including =tmux attach= typed
inside an ssh session), the running shell buffer becomes the controller, the
first pane materializes *in the same window* the shell was in (sized to your
Emacs window), and from there the tab-line lets you navigate sibling panes.

When tmux exits control mode (=M-x ghostel-tmux-detach=, =C-t d= inside the pane,
or any path that ends the DCS), every window showing a pane buffer flips back
to the original shell buffer - pane buffers are sealed as read-only snapshots.

Non-attaching tmux subcommands (=tmux ls=, =tmux kill-server=, =tmux capture-pane=,
...) pass through unchanged.  The wrapper ships for bash, zsh, fish, and
nushell.  Bypass it explicitly with =command tmux ...= (=^tmux ...= in nushell) or
by exporting =GHOSTEL_TMUX_NO_CC=1= before sourcing the shell integration.

The Emacs side hook is on by default.  Disable with:

#+begin_src emacs-lisp
(setq ghostel-tmux-auto-detect nil)
#+end_src

*** Prefix bindings
:properties:
:custom_id: tmux-prefix-bindings
:end:

Pane buffers bind =C-t= (customizable via =ghostel-tmux-prefix-key=, e.g. to =C-b=)
as a tmux-style prefix.  These translate into tmux control-mode commands sent
over the active session, so they work the same whether you started tmux via
=ghostel-tmux-new= or by typing =tmux= in a shell.

| Key sequence | tmux command          | Action                                        |
|--------------+-----------------------+-----------------------------------------------|
| =C-t "=        | =split-window -v=       | Split current pane horizontally (new below). |
| =C-t %=        | =split-window -h=       | Split current pane vertically (new right).   |
| =C-t c=        | =new-window=            | Create a new window.                         |
| =C-t x=        | =kill-pane=             | Kill the current pane.                       |
| =C-t &=        | =kill-window=           | Kill the current window.                     |
| =C-t n=        | =next-window=           | Cycle to the next window.                    |
| =C-t p=        | =previous-window=       | Cycle to the previous window.                |
| =C-t o=        | =select-pane -t :.+=    | Cycle to the next pane.                      |
| =C-t ,=        | =rename-window=         | Prompt and rename the current window.       |
| =C-t $=        | =rename-session=        | Prompt and rename the current session.      |
| =C-t z=        | =resize-pane -Z=        | Toggle zoom on the current pane.            |
| =C-t 0= ... =9=  | =select-window -t :N=   | Jump to window N.                            |
| =C-t :=        | (prompt for any cmd)  | Run an arbitrary tmux command.               |
| =C-t d=        | -                     | Detach (=ghostel-tmux-detach=).                |
| =C-t ?=        | -                     | Describe the prefix map.                     |
| =C-t C-t=      | -                     | Send a literal prefix byte to the program.   |

The map is =ghostel-tmux-prefix-map=; rebind individual keys however you like.
Switching panes via the Emacs keymap (=C-c C-n= / =C-c C-p= / =C-c C-s=) also works,
as does clicking a tab in the tab-line.

*** Tab line
:properties:
:custom_id: tmux-tab-line
:end:

Each pane buffer enables =tab-line-mode= and lists the session's other panes as
tabs.  Click a tab - or hit =C-c C-n= / =C-c C-p= - to switch panes in the same
window.

*** Sizing
:properties:
:custom_id: tmux-sizing
:end:

Pane terminals are sized to the actual Emacs window they're displayed in.  When
the window is resized (or a pane is first displayed), ghostel-tmux sends
=refresh-client -C cols,rows= to tmux so the server lays out the panes for the
real client size, then resizes the ghostel terminal to match.

*** Customization
:properties:
:custom_id: tmux-customization
:end:

| Option                              | Effect                                                                              |
|-------------------------------------+-------------------------------------------------------------------------------------|
| =ghostel-tmux-program=                | tmux executable.  Default ="tmux"=.                                                  |
| =ghostel-tmux-prefix-key=             | Prefix key in pane buffers.  Default ="C-t"=.                                        |
| =ghostel-tmux-history-lines=          | Scrollback lines to backfill via =capture-pane= on attach.  Default =10000=.          |
| =ghostel-tmux-default-rows= / =-cols=   | Initial pane terminal size before the layout-resize fires.  Default =40= x =120=.     |
| =ghostel-tmux-auto-detect=            | Enable auto-takeover when a ghostel shell enters tmux =-CC=.  Default =t=.            |
| =ghostel-tmux-buffer-name-function=   | Customize pane buffer naming.  Called with =(SESSION WINDOW PANE-ID)=.               |
...
terminal-owned, so those edits are either rejected (copy and Emacs modes) or
picked up by insert forwarding and sent to the PTY mid-composition.
...
| =M-x ghostel-readonly-enter=           | Enter the read-only mode picked by =ghostel-readonly-default-mode=.         |
