...
| =ghostel-query-before-killing=         | =auto=                                          | Confirm before killing a live buffer / =save-buffers-kill-emacs=: =t=, =nil=, or =auto= (while something other than the shell holds the terminal's foreground).                                                                                          |
...
| =ghostel-foreground-change-functions=  | =nil=                                           | Hook run with =(BUFFER PGID COMM)= when the PTY's foreground process group changes.  Needs no shell integration; only fires while the buffer is displayed.                                                 |
