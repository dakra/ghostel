...
=ghostel-consult= picks a Ghostel terminal through [[https://github.com/minad/consult][consult]] with *live preview*:
...
- =M-x ghostel-consult-buffer= - pick any Ghostel terminal.
- =M-x ghostel-consult-project-buffer= - pick a terminal in the current project.

Candidates are ordered for switching (recently-used first, the current buffer last).
...
The sources are also exposed for the global lists.  Since Ghostel buffers
already appear in =consult-buffer='s default /Buffer/ source, add the =-hidden=
variants so they do not duplicate the default view and are summoned via the =g=
narrow key:

#+begin_src emacs-lisp
(with-eval-after-load 'consult
  (add-to-list 'consult-buffer-sources 'ghostel-consult-source-hidden t)
  (add-to-list 'consult-project-buffer-sources
               'ghostel-consult-project-source-hidden t))
#+end_src

=consult= is a /soft/ dependency: it is required lazily only when these commands
run, so loading =ghostel-consult= does not pull in consult until you use it.
...
(let ((default-directory "/tmp/"))    ; ... in a specific directory (let-bind, as usual)
...
but without that command's buffer reuse; =ghostel-exec= runs a single program in
a real TTY with no shell integration.  Both read =default-directory=, so
=let=-bind it to spawn elsewhere.  =ghostel-project-buffer-list= signals a
=user-error= when there is no current project.
