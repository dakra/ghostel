...
Both run the =ghostel-notification-functions= hook with =(TITLE BODY)= and the
terminal buffer current.  The default member, =ghostel-default-notify=, uses the
[[https://github.com/jwiegley/alert][alert]] package when installed - it picks a sensible backend per platform
(=osascript= on macOS, libnotify on Linux, Growl, terminal-notifier, etc.) and is
configurable via =alert-default-style=.  Install it from MELPA with
=M-x package-install RET alert RET=.  When =alert= is not available, it falls back
to =message=, which only appears in the echo area.

Remove the default member to silence notifications, or add your own to route
them elsewhere:

#+begin_src emacs-lisp
(remove-hook 'ghostel-notification-functions #'ghostel-default-notify)
(add-hook 'ghostel-notification-functions
          (lambda (title body)
...
percentage.  Ghostel runs the =ghostel-progress-functions= hook with =(STATE
...
The default member is =ghostel-spinner-progress= when spinner.el is on the
=load-path= at ghostel load time, otherwise =ghostel-default-progress=.  Pin a
specific handler explicitly:

#+begin_src emacs-lisp
;; Pin to spinner (errors with a hint if spinner.el isn't installed):
(setq ghostel-progress-functions '(ghostel-spinner-progress))
;; Or stay on the plain text indicator:
(setq ghostel-progress-functions '(ghostel-default-progress))
...
*** System taskbar
:properties:
:custom_id: system-taskbar
:end:
#+cindex: taskbar

On Emacs 31+, =ghostel-taskbar-mode= (from =ghostel-taskbar.el=) mirrors
terminal activity on the system taskbar icon - the macOS Dock, the
MS-Windows taskbar, or a GNU/Linux launcher implementing the Unity D-Bus
spec (see the built-in =system-taskbar= package for setup details):

- OSC 9;4 progress reports become a progress bar on the Emacs icon.
  Since the icon is global, the bar tracks the last terminal that
  reported determinate progress.
- A shell command that ran for at least
  =ghostel-taskbar-attention-min-duration= seconds (default 5) and
  finishes while no Emacs frame has focus bounces/flashes the icon -
  with =critical= urgency when the command failed.  Requires OSC 133
  shell integration.
- Desktop notifications (OSC 9 / OSC 777) and the terminal bell also
  request attention while Emacs is unfocused.

#+begin_src emacs-lisp
(ghostel-taskbar-mode)
#+end_src

The underlying hooks are available for your own integrations
independent of =ghostel-taskbar-mode=: =ghostel-bell-functions=,
=ghostel-notification-functions=, =ghostel-progress-functions=,
=ghostel-command-start-functions=, and
...
The 16 ANSI colors are defined as Emacs faces inheriting from =ansi-color-*=:
...
Themes that customize =ansi-color-*= faces automatically apply.  Customize
...
Bold and italic text inherit the =ghostel-bold= and =ghostel-italic= faces,
which inherit =ansi-color-bold= and =ansi-color-italic=, and through them
Emacs's =bold= and =italic=.
...
| Variable                      | Default                                             | Description                                                     |
|-------------------------------+-----------------------------------------------------+-----------------------------------------------------------------|
| =ghostel-notification-functions= | =(ghostel-default-notify)=                            | Hook for OSC 9 / OSC 777 notifications.                         |
| =ghostel-progress-functions=     | spinner if available, else =ghostel-default-progress= | Hook for OSC 9;4 progress.                                      |
| =ghostel-bell-functions=         | =(ghostel-ding)=                                      | Hook for the terminal bell.                                     |
...
#+findex: consult-ghostel-mode
=consult-ghostel-mode= registers hidden sources in =consult-buffer= and
=consult-project-buffer=: Ghostel buffers already appear in the default /Buffer/
view, so they stay invisible there until the =g= narrow key summons them
exclusively.  The =g= narrow key (or any other source property) can be changed
with =consult-customize=, e.g.
~(consult-customize consult-ghostel-source-hidden :narrow ?t)~.

The mode also makes =consult-line= match across soft line wraps in Ghostel
buffers: rows joined by wrap newlines become one search candidate, so a path or
command that wrapped mid-word is still found.  It also adds a /Ghostel/ group
to =consult-bookmark=, so the =g= narrow key restricts the candidates to
